...
| `pin-version` | No | `"false"` | `true` / `false` を指定。`true` の場合は `--pin-version` を付与。`environment-id` 指定時のみ指定可能。同期先を「常に最新バージョンを使う」のままにせず、同期元がその時点で使っているバージョンに固定する。`@basemachina/cli` 1.5.0 以降が必要 |
